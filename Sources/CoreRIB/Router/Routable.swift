...
public protocol Routable: AnyObject {
    ...
    var state: RouterState { get }
    var statePublisher: ValuePublisher<RouterState> { get }
    var eventPublisher: ValuePublisher<RouterEvent> { get }
    @MainActor func attach(_ child: Routable) async
    @MainActor func detach(_ identifier: RouterIdentifier) async
}
